...
def eng_s(rates_frame, rsi_per, rsi_val):
    ...
    while i < ldf:
        if rates_frame[rsi_per][i] > rsi_val and rates_frame['ema20'][i] > rates_frame['ema50'][i]:
            flag_rsi3 = 1
        if flag_rsi3 == 1  and rates_frame['low'][i - 1] > \
                rates_frame['close'][i] and rates_frame[rsi_per][i] < 75 :
            flag_n = i
            flesh_U = 1
            flag_rsi3 = 0
        if flag_rsi3 == 1 and rates_frame[rsi_per][i] < 75:
            flag_rsi3 = 0
        if flesh_U == 1 and (ldf - flag_n > 8 or
                             rates_frame['close'][i] > rates_frame['open'][flag_n] or rates_frame['close'][i] >
                             rates_frame['close'][
                                 flag_n - 1]):
            flesh_U = 0
        ...
        if rates_frame[rsi_per][i] < 100 - rsi_val and rates_frame['ema20'][i] < rates_frame['ema50'][i]:
            flag_rsi3_r = 1
        if flag_rsi3_r == 1  and rates_frame['high'][
            i - 1] < rates_frame['close'][i] :
            flag_nr = i
            flesh_D = 1
            flag_rsi3_r = 0
        if flag_rsi3_r == 1 and rates_frame[rsi_per][i] > 25:
            flag_rsi3_r = 0
        if flesh_D == 1 and (
                ldf - flag_nr > 8 or rates_frame['close'][i] < rates_frame['open'][flag_nr] or rates_frame['close'][i] <
...
def tenkanH4(currency, tft, tftup):
    ...
    rates_frameup['%Kup'] = (rates_frameup['closeup'] - rates_frameup['14-lowup']) * 100 / (
            rates_frameup['14-highup'] - rates_frameup['14-lowup'])
    ...
    rates_frame['tenkan_sen'] = rates_frame['close'].ewm(span=12, adjust=False).mean()
    rates_frame = pd.merge_ordered(rates_frame, rates_frameup, fill_method="ffill")
    rates_frame = rates_frame.drop(
        ['tick_volume', 'spread', 'real_volume', '14-high', '14-low', '%K', '%D', '14-highup', '14-lowup', '%Kup',
         '%Dup'], axis=1)
    ...
    vin = False
    while i < ldf:
        if rates_frame['RSI3'][i] > 87 and rates_frame['closeup'][i] > rates_frame['ema50up'][i] and \
                rates_frame['closeup'][i] > rates_frame['ema20up'][i] \
                and rates_frame['close'][i] > rates_frame['ema50'][i] and rates_frame['close'][i] > \
                rates_frame['ema20'][i]:
            flag_rsi3_n = i
        i += 1
    if flag_rsi3_n is not None:
        ...
        flag_rsi3 = 1
        while i < ldf:
            if rates_frame['stock'][i] < 25 or rates_frame['RSI14up'][i] < 25 or rates_frame['RSI14up'][
                i] > 75:  # or rates_frame['tenkan_sen'][i] > rates_frame['close'][i]
                flag_rsi3 = 0
            if rates_frame['stock'][i] > 25 and (rates_frame['RSI14up'][i] < 25 or rates_frame['RSI14up'][
                i] > 75):  # or rates_frame['tenkan_sen'][i] > rates_frame['close'][i]
                flag_rsi3 = 2
            if rates_frame['stock'][i] < 25 and rates_frame['RSI14'][i] > 55.5 and (
                    rates_frame['RSI14up'][i] > 25 and rates_frame['RSI14up'][
                i] < 75):  # or rates_frame['tenkan_sen'][i] > rates_frame['close'][i]
                flag_O = i
            if rates_frame['stock'][i] < 25 and rates_frame['RSI14'][i] > 55.5 and (
                    rates_frame['RSI14up'][i] < 25 or rates_frame['RSI14up'][
                i] > 75):  # or rates_frame['tenkan_sen'][i] > rates_frame['close'][i]
                flag_Oj = i
            i += 1
        i = flag_rsi3_n + 1
    ...
    vinr = False
    while i < ldf:
        if rates_frame['RSI3'][i] < 13 and rates_frame['closeup'][i] < rates_frame['ema50up'][i] and \
                rates_frame['closeup'][i] < rates_frame['ema20up'][i] \
                and rates_frame['close'][i] < rates_frame['ema50'][i] and rates_frame['close'][i] < \
                rates_frame['ema20'][i]:
            flag_rsi3r_n = i
    ...
    if flag_rsi3r_n is not None:
        ...
        flag_rsi3r = 1
        while i < ldf:
            if rates_frame['stock'][i] > 75 or (rates_frame['RSI14up'][i] > 75 or rates_frame['RSI14up'][
                i] < 25):  # or rates_frame['tenken_sen'][i] < rates_frame['close'][i]
                flag_rsi3r = 0
            if rates_frame['stock'][i] < 75 and (rates_frame['RSI14up'][i] > 75 or rates_frame['RSI14up'][
                i] < 25):  # or rates_frame['tenken_sen'][i] < rates_frame['close'][i]
                flag_rsi3r = 2
            if rates_frame['stock'][i] > 75 and rates_frame['RSI14'][i] < 44.5 and (
                    rates_frame['RSI14up'][i] > 25 and rates_frame['RSI14up'][
                i] < 75):  # or rates_frame['tenkan_sen'][i] > rates_frame['close'][i]
                flag_Or = i
            if rates_frame['stock'][i] > 75 and rates_frame['RSI14'][i] < 44.5 and (
...
def tire_vert(currency, TF, TFup):
    ...
    while (rates is None) and (ratesup is None):
        ...
        if TF == mt5.TIMEFRAME_H1:
            ratesupup = mt5.copy_rates_from(currency, mt5.TIMEFRAME_D1, datetime(2025, 2, 6), 100)
        # shut down connection to the MetaTrader 5 terminal
        mt5.shutdown()
    rates_frame = pd.DataFrame(rates)
    rates_frame['time'] = pd.to_datetime(rates_frame['time'], unit='s')
    ...
    if TF == mt5.TIMEFRAME_M1:
        ...
        rates_frameupup = rates_frameupup.rename(
            columns={"close": "closeupup", "open": "openupup", "low": "lowupup", "high": "highupup"})
    ...
    if TF == mt5.TIMEFRAME_M5:
        ...
        rates_frameupupup = rates_frameupupup.rename(
            columns={"close": "closeupupup", "open": "openupupup", "low": "lowupupup", "high": "highupupup"})
        rates_frameupupupup = pd.DataFrame(ratesupupupup)
        rates_frameupupupup = rates_frameupupupup.rename(
            columns={"close": "closeupupupup", "open": "openupupupup", "low": "lowupupupup", "high": "highupupupup"})
        rates_frameupup['time'] = pd.to_datetime(rates_frameupup['time'], unit='s')
    ...
    rates_frame = pd.merge_ordered(rates_frame, rates_frameup, fill_method="ffill")
    if TF == mt5.TIMEFRAME_M1 or TF == mt5.TIMEFRAME_M5 or TF == mt5.TIMEFRAME_M15 or TF == mt5.TIMEFRAME_H1:
...
def trt(rates_frame):
    ...
    contre = False
    while i < ldf:
        if TF == mt5.TIMEFRAME_M1 or TF == mt5.TIMEFRAME_H1 or TF == mt5.TIMEFRAME_M15:
            if rates_frame['RSI3'][i] > 87 and rates_frame['closeup'][i] > rates_frame['ema50up'][i] and \
                    rates_frame['closeup'][i] > rates_frame['ema20up'][i] \
                    and rates_frame['close'][i] > rates_frame['ema50'][i] and rates_frame['close'][i] > \
                    rates_frame['ema20'][i] and rates_frame['closeupup'][i] > rates_frame['ema50upup'][i] and \
                    rates_frame['closeupup'][i] > rates_frame['ema20upup'][i]:
                flag_rsi3_n = i
            ...
            if rates_frame['RSI3'][i] > 87 and rates_frame['closeup'][i] > rates_frame['ema50up'][i] and \
                    ...
                    rates_frame['closeupup'][i] > rates_frame['ema20upup'][i] \
                    and (
                    rates_frame['closeupupup'][i] > rates_frame['ema50upupup'][i] and rates_frame['closeupupup'][i] >
                    rates_frame['ema20upupup'][i] \
                    or rates_frame['closeupupupup'][i] > rates_frame['ema50upupupup'][i] and
    ...
    if flag_rsi3_n is not None:
        ...
        flag_rsi3 = 1
        while i < ldf:
            if rates_frame['stock'][i] < 25 or rates_frame['RSI14up'][i] < 25 or rates_frame['RSI14up'][i] > 75 or \
                    rates_frame['tenkan_sen'][i] > rates_frame['close'][i]:
                flag_rsi3 = 0
            if rates_frame['stock'][i] > 25 and (rates_frame['RSI14up'][i] < 25 or rates_frame['RSI14up'][i] > 75) and \
                    rates_frame['tenkan_sen'][i] < rates_frame['close'][
                i]:  # or rates_frame['tenkan_sen'][i] > rates_frame['close'][i]
                flag_rsi3 = 2
            if rates_frame['stock'][i] < 25 and rates_frame['RSI14'][i] > 55.5 and (
                    rates_frame['RSI14up'][i] > 25 and rates_frame['RSI14up'][i] < 75) and rates_frame['tenkan_sen'][
                i] < rates_frame['close'][i]:  # or rates_frame['tenkan_sen'][i] > rates_frame['close'][i]
                flag_O = i
            if rates_frame['stock'][i] < 25 and rates_frame['RSI14'][i] > 55.5 and (
                    rates_frame['RSI14up'][i] < 25 or rates_frame['RSI14up'][i] > 75) and rates_frame['tenkan_sen'][i] < \
                    rates_frame['close'][i]:  # or rates_frame['tenkan_sen'][i] > rates_frame['close'][i]
                flag_Oj = i
            i += 1
        i = flag_rsi3_n + 1
        if flag_rsi3 == 2:
    ...
    while i < ldf:
        if TF == mt5.TIMEFRAME_M1 or TF == mt5.TIMEFRAME_H1 or TF == mt5.TIMEFRAME_M15:
            ...
            if rates_frame['RSI3'][i] < 13 and rates_frame['closeup'][i] < rates_frame['ema50up'][i] and \
                    ...
                    rates_frame['ema20'][i] and rates_frame['closeupup'][i] < rates_frame['ema50upup'][i] and \
                    rates_frame['closeupup'][i] < rates_frame['ema20upup'][i] \
                    and rates_frame['closeupupup'][i] < rates_frame['ema50upupup'][i] and rates_frame['closeupupup'][
                i] < rates_frame['ema20upupup'][i]:
                flag_rsi3r_n = i
        else:
            if rates_frame['RSI3'][i] < 13 and rates_frame['closeup'][i] < rates_frame['ema50up'][i] and \
                    rates_frame['closeup'][i] < rates_frame['ema20up'][i] \
    ...
    if flag_rsi3r_n is not None:
        ...
        flag_rsi3r = 1
        while i < ldf:
            if rates_frame['stock'][i] > 75 or (rates_frame['RSI14up'][i] > 75 or rates_frame['RSI14up'][i] < 25) or \
                    rates_frame['tenkan_sen'][i] < rates_frame['close'][
                i]:  # or rates_frame['tenken_sen'][i] < rates_frame['close'][i]
                flag_rsi3r = 0
            if rates_frame['stock'][i] < 75 and (rates_frame['RSI14up'][i] > 75 or rates_frame['RSI14up'][i] < 25) and \
                    rates_frame['tenkan_sen'][i] > rates_frame['close'][
                i]:  # or rates_frame['tenken_sen'][i] < rates_frame['close'][i]
                flag_rsi3r = 2
            if rates_frame['stock'][i] > 75 and rates_frame['RSI14'][i] < 44.5 and (
...
def findzones(currency, TF, TFup):
    ...
    rates_frame['RSI14'] = computeRSI(rates_frame['close'], 14)
    ll = len(rates_frame)
    sma5 = (rates_frame['close'][ll - 2] + rates_frame['close'][ll - 3] + rates_frame['close'][ll - 4] +
            rates_frame['close'][ll - 5] + rates_frame['close'][ll - 6]) / 5
    ...
    real_Ex = [rates_frame['RSI14'][len(rates_frame) - 1], rates_frame['ema50'][len(rates_frame) - 1],
               rates_frame['ema20'][len(rates_frame) - 1], sma5, sma3]
    df = rates_frame
    dfc = df['close']
    ldf = len(df) - 1
    lows = [0] * (ldf + 1)
...
def free(zones, last_close, df, real_DT, real_Ex, real_Ex1, real_last_close_M15, pivots, currency):
    ...
    if not inside_zone:
        ...
        if len(l) > 0:
            ...
            il1 = 9999999
        if if1 < il1 and real_DT == 'v' and real_Ex[0] < 75 and real_last_close_M15 > real_Ex1[3] and real_Ex1[1] < \
                real_Ex1[2] and real_Ex1[2] < real_Ex1[3] and real_Ex1[3] < real_Ex1[
            4]:  # and real_last_close_M15 > real_Ex1[1] and real_Ex1[1] > real_Ex1[2] and real_Ex1[2] > real_Ex1[3] and real_Ex1[3] > real_Ex1[4]
            last_touche = 'l'
        elif if1 < il1 and real_DT == 'v' and real_Ex[0] > 75 and real_last_close_M15 > real_Ex1[3] and real_Ex1[1] < \
                real_Ex1[2] and real_Ex1[2] < real_Ex1[3] and real_Ex1[3] < real_Ex1[4]:
            last_touche = 'E'
        elif if1 > il1 and real_DT == 'r' and real_Ex[0] > 25 and real_last_close_M15 < real_Ex1[3] and real_Ex1[1] > \
                real_Ex1[2] and real_Ex1[2] > real_Ex1[3] and real_Ex1[3] > real_Ex1[4]:  #
            last_touche = 'f'
        elif if1 > il1 and real_DT == 'r' and real_Ex[0] < 25 and real_last_close_M15 < real_Ex1[3] and real_Ex1[1] > \
                real_Ex1[2] and real_Ex1[2] > real_Ex1[3] and real_Ex1[3] > real_Ex1[4]:
...
def freedom(currency):
    zones = []
    pivots = []
    for tfs in [[mt5.TIMEFRAME_M15, mt5.TIMEFRAME_H1], [mt5.TIMEFRAME_H1, mt5.TIMEFRAME_H4],
...
currencys = ['EURUSD', 'GBPUSD', 'USDCAD', 'USDJPY', 'GOLD', 'WTI', '#USSPX500', '#USNDAQ100', '#US30',
             '#Euro50',
             'EURGBP']  # '@EP','@ENQ', '@YM', '@RTY', '@DD', '@DSX', 'XAUUSD', '@CLE', '@DB', 'TYAH21','EURUSD', 'GBPUSD', 'USDCAD','USDJPY','EURGBP'
...
while True:
    ...
    if dt.second == 1:
        ...
        data = {'Zone': last_touches, 'I': currencys, 'M1': M2[0][1], 'M5': M2[1][1], 'M15': M2[2][1], 'H1': M2[3][1],
                'H4': M2[4][1],
                'D1': M2[5][1]}
        df = pd.DataFrame(data)
        ...
        for i in range(len(df)):
            item = Item(Zone=df['Zone'][i], name=currencys[i], M1=df['M1'][i], M5=df['M5'][i], M15=df['M15'][i],
                        H1=df['H1'][i], H4=df['H4'][i], D1=df['D1'][i])
            db.session.add(item)
            db.session.commit()
        for i in range(len(df1)):
            item2 = Ud(name=currencys[i], M1=df1['M1'][i], M5=df1['M5'][i], M15=df1['M15'][i], H1=df1['H1'][i],
                       H4=df1['H4'][i], D1=df1['D1'][i])
